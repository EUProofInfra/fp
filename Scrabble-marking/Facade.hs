...
import safe qualified Scrabble
import Sowpods
...
testAttempted x = try $ withOurTimeout $ evaluate $ rnf x
  where
    withOurTimeout cmd = do
      mresult <- timeout ourTimeout cmd
      case mresult of
        Just result -> return result
        Nothing -> error "timeout inside testAttempted"

ourTimeout :: Int
ourTimeout = 1000000 * 60 -- 60 seconds
